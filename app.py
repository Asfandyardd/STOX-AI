...
def get_company(symbol):
    try:
        return jsonify(get_live_market_data(symbol))
    except Exception as e:
...
def analyze_stock(symbol):
    try:
        ...
        prompt = f"""
Analyze asset '{symbol.upper()}' based on these exact live session parameters:
...
{{
    ...
    "marketSummary": "<2 sentences mentioning current price level>",
